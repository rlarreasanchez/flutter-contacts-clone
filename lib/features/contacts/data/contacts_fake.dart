import 'package:contactos_app/features/contact/models/contact_model.dart';

List<ContactModel> contactsFake = [
  ContactModel(
    id: 1,
    imgUrl: '',
    name: 'Amalia Amigo',
    email: 'amalia.amigo@email.com',
  ),
  ContactModel(
    id: 2,
    imgUrl: '',
    name: 'Aitana Estévez',
    email: 'aitana.estévez@email.com',
  ),
  ContactModel(
    id: 3,
    imgUrl:
        'https://images.unsplash.com/photo-1527980965255-d3b416303d12?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=880&q=80',
    name: 'Alberto Pérez',
    email: 'dk.pr@email.com',
    favorite: true,
  ),
  ContactModel(
    id: 4,
    imgUrl:
        'https://images.unsplash.com/photo-1580489944761-15a19d654956?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=761&q=80',
    name: 'Antonio Suarez',
    email: 'antonio.suarez@email.com',
  ),
  ContactModel(
    id: 5,
    imgUrl:
        'https://images.unsplash.com/photo-1580489944761-15a19d654956?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=761&q=80',
    name: 'Alberto Punzon',
    email: 'alberto.punzon@email.com',
  ),
  ContactModel(
    id: 6,
    imgUrl: '',
    name: 'Alberto Márquez',
    email: 'alberto.márquez@email.com',
    favorite: true,
  ),
  ContactModel(
    id: 7,
    imgUrl:
        'https://images.unsplash.com/photo-1580489944761-15a19d654956?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=761&q=80',
    name: 'Fernando Albarez',
    email: 'fernando.albarez@email.com',
  ),
  ContactModel(
    id: 8,
    imgUrl: '',
    name: 'Esteban Albarez',
    email: 'esteban.albarez@email.com',
  ),
  ContactModel(
      id: 9,
      imgUrl:
          'https://images.unsplash.com/photo-1570295999919-56ceb5ecca61?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=880&q=80',
      name: 'Sergio López',
      email: 'sergio.lópez@email.com',
      favorite: true),
  ContactModel(
      id: 10,
      imgUrl:
          'https://images.unsplash.com/photo-1633332755192-727a05c4013d?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=880&q=80',
      name: 'Pablo Caballero',
      favorite: true),
  ContactModel(
      id: 11,
      imgUrl:
          'https://images.unsplash.com/photo-1527980965255-d3b416303d12?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=880&q=80',
      name: 'Pedro Sánchez',
      email: 'pedro.sánchez@email.com',
      favorite: true),
  ContactModel(
    id: 12,
    imgUrl:
        'https://images.unsplash.com/photo-1527980965255-d3b416303d12?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=880&q=80',
    name: 'Rafaela Pascual',
    email: 'rafaela.pascual@email.com',
  ),
  ContactModel(
      id: 13,
      imgUrl:
          'https://images.unsplash.com/photo-1527980965255-d3b416303d12?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=880&q=80',
      name: 'Fabio Palenzuela',
      email: 'fabio.palenzuela@email.com',
      favorite: true),
  ContactModel(
    id: 14,
    imgUrl:
        'https://images.unsplash.com/photo-1527980965255-d3b416303d12?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=880&q=80',
    name: 'Carlos Laredo',
    email: 'carlos.laredo@email.com',
  ),
  ContactModel(
      id: 15,
      imgUrl:
          'https://images.unsplash.com/photo-1527980965255-d3b416303d12?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=880&q=80',
      name: 'Nicolas González',
      email: 'nicolas.gonzález@email.com',
      favorite: true),
  ContactModel(
    id: 16,
    imgUrl:
        'https://images.unsplash.com/photo-1527980965255-d3b416303d12?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=880&q=80',
    name: 'Carlos Martínez',
    email: 'carlos.martínez@email.com',
  )
];
